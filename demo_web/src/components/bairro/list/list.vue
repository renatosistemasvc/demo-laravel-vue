    ...
    <div>
        <el-row class="u-mt-10 u-mb-20">
            <el-col :xs="24" :sm="16" :md="16">
                <el-input v-model="filtro['nome']" placeholder="Pesquisar bairro..." @keyup.native.enter="loadingBairros"></el-input>
            </el-col>
            <el-col :xs="24" :sm="8" :md="8" class="u-text-right">
                <router-link to="/admin/bairro/cadastro" class="btn-new-register">
                    <el-button type="primary">NOVO BAIRRO</el-button>
                </router-link>
            </el-col>
        </el-row>
                ...
                <tbody>
                    <tr v-for="item in items" :key="item.id">
                        ...
                        <td class="u-text-center acao"> 
                            <router-link :to="'/admin/bairro/edit/'+ item.id" class="link-icon" style="color: rgba(0, 0, 0, 0.87) !important;">
                                <el-button type="info" title="Editar" icon="el-icon-edit" size="mini"></el-button>
                            </router-link>
                            <el-button @click="deleteBairro(item.id)" title="Excluir" type="danger" icon="el-icon-delete" size="mini"></el-button>
                        </td>
                    </tr>
                </tbody>
                <tfoot>
...
export default {
    ...
    methods: {
        loadingBairros(){
            ...
            this.$axios.post(`${this.$store.state.config.baseUrlApi}/api/bairro/getAllPaginate?page=` + this.paginate.page, this.filtro, this.$store.getters.getHeaderAuth).then(response => {
        ...
        deleteBairro(id){
            ...
            this.$msgbox.confirm('Tem certeza que deseja excluir esta bairro?', 'Atenção', { confirmButtonText: 'Sim', cancelButtonText: 'Não', type: 'warning' }).then(() => {

                window.eventBus.$emit('loading', true);
                    
                this.$axios.delete(`${this.$store.state.config.baseUrlApi}/api/bairro/delete/` + id, this.$store.getters.getHeaderAuth).then(response => {
