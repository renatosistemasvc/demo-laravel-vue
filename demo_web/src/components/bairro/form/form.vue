<template>
    <div>
        ...
        <el-form @submit.native.prevent="onSubmitBairro" style="min-height: 130px;" v-loading="editBairro && !dados.id" :model="dados" ref="dados" label-position="top">
            <div v-if="!editBairro || dados.id">
                ...
                <el-row :gutter="10">
                    <el-col :xs="24">
                        <el-form-item class="u-text-right u-mt-15">
                            <el-button type="primary" id="btn-bairro" native-type="submit">SALVAR</el-button>
                        </el-form-item>
                    </el-col>
                </el-row>
            </div>
...
export default {
    ...
    methods:{
        getBairroById(id){
            ...
            this.$axios.get(`${this.$store.state.config.baseUrlApi}/api/bairro/getById/` + id, this.$store.getters.getHeaderAuth).then(response => {
        ...
        onSubmitBairro(){
            ...
            document.getElementById("btn-bairro").focus();
            ...
            this.$refs.dados.validate((valid) =>{ if(valid){
                ...
                if(this.editBairro){
                    ...
                    this.$axios.post(`${this.$store.state.config.baseUrlApi}/api/bairro/update/` + this.dados.id, this.dados, this.$store.getters.getHeaderAuth).then(response => {

                        window.eventBus.$emit('loading', false);
                        window.eventBus.$emit('alert', {msg: 'Sucesso! - Operação realizada com Sucesso!', type: 'success'});
                        
                        this.$emit('onSubmitBairro');

                    }).catch((error) => { window.eventBus.$emit('loading', false); this.$store.commit('throwException', error); });

                }else{

                    this.$axios.post(`${this.$store.state.config.baseUrlApi}/api/bairro/store`, this.dados, this.$store.getters.getHeaderAuth).then(response => {
        ...
        paramId: {
            ...
            handler (value) {
                ...
                if(value){
                    ...
                    this.getBairroById(value);
                    this.editBairro = true;
                } 
            }
        },
    }
