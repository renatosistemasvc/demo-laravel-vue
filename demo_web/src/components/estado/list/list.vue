    ...
    <div>
        <el-row class="u-mt-10 u-mb-20">
            <el-col :xs="24" :sm="16" :md="16">
                <el-input v-model="filtro['nome']" placeholder="Pesquisar estado..." @keyup.native.enter="loadingEstados"></el-input>
            </el-col>
            <el-col :xs="24" :sm="8" :md="8" class="u-text-right">
                <router-link to="/admin/estado/cadastro" class="btn-new-register">
                    <el-button type="primary">NOVO ESTADO</el-button>
                </router-link>
            </el-col>
        </el-row>
            ...
            <table v-loading="!statusList" class="table table-striped">
                <thead>
                    <tr>
                        ...
                        <th>Nome</th>
                        <th class="hidden-mobile u-text-center">UF</th>
                ...
                <tbody>
                    <tr v-for="item in items" :key="item.id">
                        ...
                        <td>{{ item.nome | truncate(30, '...') }}</td>
                        <td class="hidden-mobile u-text-center">{{ item.uf }}</td>
                        <td class="hidden-mobile u-text-center">{{ (item.situacao) ? 'Ativo' : 'Inativo' }}</td>
                        <td class="u-text-center acao"> 
                            <router-link :to="'/admin/estado/edit/'+ item.id" class="link-icon" style="color: rgba(0, 0, 0, 0.87) !important;">
                                <el-button type="info" title="Editar" icon="el-icon-edit" size="mini"></el-button>
                            </router-link>
                            <el-button @click="deleteEstado(item.id)" title="Excluir" type="danger" icon="el-icon-delete" size="mini"></el-button>
                        </td>
                    </tr>
                </tbody>
                <tfoot>
...
export default {
    ...
    methods: {
        loadingEstados(){
            ...
            this.$axios.post(`${this.$store.state.config.baseUrlApi}/api/estado/getAllPaginate?page=` + this.paginate.page, this.filtro, this.$store.getters.getHeaderAuth).then(response => {
        ...
        deleteEstado(id){
            ...
            this.$msgbox.confirm('Tem certeza que deseja excluir esta estado?', 'Atenção', { confirmButtonText: 'Sim', cancelButtonText: 'Não', type: 'warning' }).then(() => {

                window.eventBus.$emit('loading', true);
                    
                this.$axios.delete(`${this.$store.state.config.baseUrlApi}/api/estado/delete/` + id, this.$store.getters.getHeaderAuth).then(response => {
